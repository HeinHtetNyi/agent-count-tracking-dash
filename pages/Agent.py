from dash import Dash, html, dash_table, dcc, callback, Input, Output, register_page
import pandas as pd
import plotly.graph_objects as go

register_page(__name__, path="/agent")

df = pd.read_csv("./preparation/agent_analysis.csv")
...
layout = [
    ...
    html.Div(id='agent-graph', style={'display': 'flex', 'flex-wrap': 'wrap', 'justify-content': 'center'})
]

@callback(
    Output(component_id='agent-graph', component_property='children'),
    Input(component_id='agent-townships', component_property='value')
)
